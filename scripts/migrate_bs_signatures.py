...
print("registrations з participant: %s" % len(regs))
for reg in regs:
    ...
    try:
        ...
        print("  read FAIL order %s: %s" % (order.id, str(e)[:80]))
        nosig += 1
        continue
    if not sig:
        nosig += 1
...
print(
    "ПІДСУМОК: migrated=%s skipped(вже є)=%s no_signature=%s no_order=%s | DRY_RUN=%s"
    % (migrated, skipped, nosig, noreg, DRY_RUN)
)
if not DRY_RUN:
