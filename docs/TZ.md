...
- **Проблема + цінність:** MEN-документація (картки, dziennik, teczka KO), RODO, Kamilka-протоколи — вручну, довго, дорого, ризиковано (штрафи/кураторіум). Портал автоматизує весь цикл: створення → продаж → кадра → проведення → звіти. Ключова ціннісна пропозиція SaaS: **«Ви робите табір — облік залиште нам»** — організація веде програму й дітей, а весь регуляторний облік (MEN/Kamilka/RODO/kuratorium/VAT-маржа) бере портал.
- **Ринок/сегмент:**
  - **Етап 1** — власна операція CampScout (перший клієнт-еталон).
  - **Етап 2 (SaaS)** — малі фірми та громадські організації, що проводять табори час від часу: **Ochotnicza Straż Pożarna (OSP)** та подібні NGO (парафії, спортклуби, стоваришення). Не куплять важкий проф-софт — куплять «під ключ, без паперової тяганини».
- **Успіх БІЗНЕСУ (заповнено CEO 2026-07-02):**
  - **Етап 1 (ціль — сезон 2027):** усі наші табори — **5 напрямків / ~11 заїздів** (базлайн 2025) — ведуться через портал; моноліт `campscout_management` відключено.
  - **Етап 2 (після):** продати SaaS зовнішнім організаціям (OSP та ін.); цільові цифри — коли етап 1 у проді.
- **Терміни (два контрольні вікна):** реєстрація сезону 2027 стартує **серпень-вересень 2026**; пік літа-2027 — **травень 2027** (пік зими невідомий, раніше).
  - **Мін. prod** (весь сезон 2027 на порталі, з зимовиськами): до **серпня-вересня 2026** (~1-2 міс від 2026-07-02).
  - **Крайній prod** (зловити основну масу літа): до **травня 2027**; тоді зимовиська 2027 підуть повз портал.
- **Відповідальний + бюджет:** власник — Volodymyr Shevchenko (Fayna Digital); бюджет внутрішній (час власника + агенти), без зовнішнього фінансування.
- **🔴 Головний бізнес-ризик:** вузьке вікно — реєстрація 2027 стартує вже за ~1-2 міс. Проґавити → сезон 2027 не на порталі → ще рік (2 сезони поспіль 95% готово, 0 у проді → 0 доходу). Загострює: блокер **R1** (майстер не створює табір) на критичному шляху — без нього немає реєстрації.
