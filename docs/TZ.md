...
**[F-RODO-4] Erasure (право на забуття).** Wizard на res.partner → анонімізація + лог action='erased' + SendPulse sync; юридичні дані (account.move/sale.order, retention 5-7 р.) НЕ видаляються — анонімізація; медичні/інцидентні — не erased (art.17(3)(e), art.89 anonimizacja). · 🔴 **wizard НЕ реалізовано** — аудит 03.08 (docs/tz-audit/2026-08-03/AUTO-STATUS.md, DRIFT): у порталі лише docstring `participant.py:1055`; у fayna_rodo_compliance лише cron анонімізації consent-логів (`_cron_anonymize_expired_consent_logs`) — це ✅, але воно НЕ erasure-wizard. Попередній «✅ Доказ [ПЕРЕВІРЕНО: grep]» був grep-збігом docstring — знято. · У черзі §8 п.20.
...
20. **Erasure-wizard [F-RODO-4]** — єдиний підтверджений DRIFT аудиту 03.08 (лупом дешевих API + гейт; звіт `docs/tz-audit/2026-08-03/AUTO-STATUS.md`): wizard на res.partner відсутній у всіх репо комплекту. ~1-2 дні.
21. **Сезонний беклог з корпусу 3 таборів 2026.** ✅ **Тема 2 (реєстр видачі ліків) РЕАЛІЗОВАНА 04.08** за «ок» власника: `models/medication.py` (camp.medication.registry + .schedule + ескалація), views (kanban медика з тач-кнопками), cron (missed→activity+SMS CRITICAL), ACL+record rules+art.9-групи, canvas-підпис згоди батьків за патерном [F-KKW-2]; 16 тестів. Прогін на живій БД test_camp 04.08 [ПЕРЕВІРЕНО: docker odoo:17.0]: повний сьют `1 failed, 5 error(s) of 254` → усі 5 errors = синтетичні legacy-таблиці, що лишились у БД від прогону 07.07 (після дропу артефактів ті самі класи `0 failed, 0 error(s) of 6`); 16/16 medication зелені; єдиний failed — відомий мовний мисматч TestAdminDashboard (окремий пункт беклогу). Некомічено — чекає «запуши». Відомий minor: `_dose_times_of_day` може дублювати слот при interval, що впирається у cap 23:00 (задокументовано, не блокер). Решта тем черги нижче. (аналіз реальних артефактів P1/P2/W лупом; синтез: DevJournal `projects/campscout/learning-corpus-2026/SYNTHESIS.md` — 8 тем, ТЗ-чернетки тем 1/2/5/6 у `tz-drafts/` з гейтом). Пріоритет власника 03.08: теми 1 (документ-на-місце + червоне світло відсутнього), 2 (реєстр видачі ліків — кейс Tsybulko), 6 (трансфер по фактурах + виїзні підписи), далі 4/8, далі 3/5/7. Кожна тема = окреме ТЗ + «ок» ([PR-6]). Факти власника, що стали вимогами: договору з перевізником може не бути (первинний доказ = фактура); zgłoszenie MEN 2026 свідомо не подавалось (формат укр. табору) → режим «без zgłoszenia» = легальна конфігурація, не помилка; відсутність документа = добрий тест-кейс, портал мусить її СВІТИТИ, а не ховати.
22. **UX-аудит усіх UI-поверхонь ЗАВЕРШЕНО 03.08** (кодове «зручність»; повний звіт: `docs/tz-audit/2026-08-03/UX-REPORT.md` — 🔴 13 · 🟡 36 · 🟢 21; несучі 🔴 перевірені другим проходом). Критерій приймання власника: **однакова робота мобільний↔десктоп, адаптація під екран, читабельність** — вердикт: кіоск ✅ близько (R4b виправлено), /my/* ❌ (canvas зміщує штрих, escort-таблиця, стилі лояльності), форми ⚠️ (iOS-zoom, підпис/resize). Черга фіксів за пріоритетом:
    - **22а 🔴 zwrot фальшивий успіх** (zwrot.html:254-272,384-388 — resp.ok не читається, «Dziękujemy» безумовно): перенести патерн rodzice (resp.ok+чесна помилка+busy). Живий клієнтський канал — НАЙПЕРШИЙ фікс; деплой через staging-гейт.
    - **22б 🔴 підписи**: порожній canvas приймається (portal_chatter.xml:244-255, portal.py:302-322 + escort) + підпис стирається resize-ом (zwrot.html:391, rodzice.html:312) + координати не масштабуються при стисканні canvas — пакет «підпис надійний на телефоні».
    - **22в 🔴 сироти й глухі кути кабінету**: /my/stories без лінків на detail (portal_templates.xml:468-492); /my/escort і /my/transport без входу з hero; portal.css:2-4 ховає .o_portal_docs (конфлікт з portal_hero.scss:82-90) — «Dzień w obozie» невидимий батькам.
    - **22г 🔴 час у UTC** на конвої/трансфері (portal_escort.xml:28,48 + portal_chatter.xml:487-521) → t-field/format_datetime у Europe/Warsaw.
    - **22д 🔴 тихі нулі тривог** admin dashboard (admin.py:129-137,241-258,345-346): except → «—»+badge «помилка даних», не 0.
    - **22е 🔴 RODO-розриви форм**: consent-модаль без «Anuluj»; rodzice обіцяє «Tylko pobierz» без кнопки; ferie2027 без жодного CTA/контакту.
    - **22ж 🟡 пакет консистентності**: 4 форми = 1 поведінка збою (busy+resp.ok+номер подання); інпути ≥16px (iOS-zoom); IBAN paste+mod-97 (validators.js вже має — підключити до zwrot); dark-mode --danger; пастка «заповнив-не-зберіг» перед підписом; переклади неекстрактованих літералів; /my/loyalty шаблон↔SCSS; R9 контекст + R5 лого + logout кіоска; beacon: clearLocal лише після resp.ok + стадія validation_failed.
    - Наскрізний інваріант (лінза T1, вимога): **усе, що людина ввела/намалювала, переживає збій мережі, поворот екрана і повторний вхід**; кожен сабміт завершується видимим підтвердженням з номером подання (R4). Фінальний доказ parity — візуальний гейт [T-5] на реальному движку (Safari RDM/симулятор), не headless.
23. **Мертва гілка `fix/r2-bilingual-switcher` з 3 червоними тестами ізоляції art. 9 — рішення власника (передано зі скану CI 11.08.2026, кодове «СІ»).** Останній прогін гілки — `28619568346` від **02.07.2026** (sha `1ed0df886`), job `Odoo unit tests (test-gate)`: **3 failed of 207**, усі три — про доступ батька до чужої дитини: `TestArt9HttpIsolation.test_parent_a_cannot_read_child_b_art9_card`, `TestPortalCampDay.test_foreign_event_redirects_to_my`, `TestRoleParentPortal.test_parent_redirected_from_foreign_camp_day`. Лінт у тому ж прогоні зелений.
    - **На main цього дефекту НЕМА** [ПЕРЕВІРЕНО 11.08: `gh run list --branch main` → останній прогін `31486861576` (sha `5b144f932`, 11:29 UTC) `success`; усі три тести присутні в коді — `tests/test_art9_http_isolation.py`, `tests/test_portal_camp_day.py`, `tests/test_role_parent_portal.py`]. Тобто це стан ТІЄЇ гілки, а не відкрита діра в проді.
    - **PR #15 закритий без merge** (`mergedAt: null`) — робота гілки не влита. Тому скіл `ci-fixer` її свідомо не чіпав: зелений на мертвій гілці — робота в нікуди, а видалення гілки заради чистого списку прогонів = глушіння симптому.
    - **Питання власнику (відкрите):** видалити гілку як закинуту — чи підняти з неї ті 3 тести окремим завданням у цей беклог? Друге має сенс лише якщо в гілці лишилась цінна логіка двомовності, не влита в main; поки це не з'ясовано, пункт не має оцінки трудомісткості.
    - Поки пункт відкритий, повний скан CI (`DevJournal/scripts/ci_sweep.py`, групування по парах гілка×workflow) щоразу показуватиме цей RED — це очікувано, не новий збій.
