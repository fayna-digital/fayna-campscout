...
FIRST = ["Adam", "Zofia", "Jan", "Maja", "Kacper", "Lena", "Filip", "Zuzia",
         "Szymon", "Hania", "Mikołaj", "Ola", "Wojtek", "Ala", "Igor", "Nadia",
         "Олег", "Соломія", "Назар", "Даринка", "Остап", "Мирослава"]
LAST = ["Nowak", "Kowalski", "Wiśniewski", "Wójcik", "Kowalczyk", "Kamiński",
        "Шевченко", "Коваленко", "Бондаренко", "Ткачук", "Мельник", "Поліщук"]
...
if existing:
    print("SEED: табір вже існує (id=%s) — пропускаю." % existing.id)
else:
    ...
    camp = env["event.event"].create({
        ...
        "date_end": datetime.combine(START + timedelta(days=DAYS), datetime.min.time()).replace(hour=18),
        "badge_format": badge,
    })
    print("SEED event id=%s badge=%s" % (camp.id, badge))

    # — Групи —
    groups = []
    for gname in GROUP_NAMES:
        groups.append(env["camp.group"].create({"name": gname, "event_id": camp.id}))
    print("SEED groups=%s" % len(groups))

    # — Батьки + діти (60), по 10 у групу, перший у групі = молодий лідер —
    n_parents = N_CHILDREN // 2
    parents = []
    for i in range(n_parents):
        parents.append(env["res.partner"].create({
    ...
    for i in range(N_CHILDREN):
        ...
        child_partner = env["res.partner"].create({
            "name": "%s %s" % (fn, ln), "is_company": False,
        })
        ...
        env["event.registration"].create({
            "event_id": camp.id, "participant_id": part.id, "partner_id": child_partner.id,
        })
    ...
    print("SEED children=%s leaders=%s parents=%s + karty(Lorem, podpisane)" % (N_CHILDREN, leaders_marked, n_parents))
    ...
    for name, role, has_krk, assigned in staff_spec:
        rec = env["camp.staff"].create({
            "name": name, "event_id": camp.id, "role": role,
            "date_from": df, "date_to": dt, "assigned_groups": assigned,
        })
        if has_krk:
            # KRK-довідка (zaświadczenie o niekaralności) verified → has_valid_krk=True → §13-допуск
            env["camp.staff.cert"].create({
                "staff_id": rec.id, "cert_type": "krk",
                "verification_status": "verified",
                "issue_date": df - timedelta(days=30),
                "expiry_date": dt + timedelta(days=365),
            })
        staff[name] = rec
    print("SEED staff=%s (KRK✓=%s)" % (len(staff), sum(1 for s in staff.values() if s.has_valid_krk)))

    wychowawcy = [staff["Wychowawca Anna (KRK ✓)"],
    ...
    print("SEED menu_days=%s (4 posiłki)" % DAYS)
    ...
    def build_day(program, d, rainy):
        ...
        day = env["camp.program.day"].create({
            "program_id": program.id, "date": the_date,
            "day_number": d + 1, "notes": LOREM_SHORT,
        })
        ...
        acts = [
            ...
            (8.0, 8.5, "Śniadanie", ""),
            (9.0, 11.0, ("Zajęcia w sali (deszcz): " if rainy else "Kąpiel morska + ") + LOREM_SHORT, LOREM),
            ...
            (18.5, 19.0, "Kolacja", ""),
            (20.0, 20.5, "Wieczorny prysznic (limit 30 miejsc) — DZIŚ myją się: " + shower_groups,
             "Połowa obozu (30 dzieci). Dni nieparzyste: %s. Dni parzyste: %s. %s"
             % (odd_groups, even_groups, LOREM_SHORT)),
            (21.5, 22.0, "Cisza nocna", ""),
        ]
        for tf, tt, title, note in acts:
            env["camp.program.activity.line"].create({
                "day_id": day.id, "time_from": tf, "time_to": tt,
                "title": title, "notes": note or LOREM_SHORT,
            })
        return day
    ...
    print("SEED programs=2 (sun+rain) × %s dni z wieczornym prysznicem (rotacja 30/30)" % DAYS)
    ...
    for w in wychowawcy:
        ...
        for d in range(3):
            day = env["camp.program.day"].create({
                "program_id": pw.id, "date": START + timedelta(days=d),
                "day_number": d + 1, "notes": "Program wychowawcy %s — %s" % (w.name, LOREM_SHORT),
            })
            env["camp.program.activity.line"].create({
                "day_id": day.id, "time_from": 16.0, "time_to": 18.0,
                "title": "Zajęcia grupy (%s): %s" % (w.name, LOREM_SHORT), "notes": LOREM,
            })
    ...
    for g in groups:
        env["fayna.camp.dziennik"].create({
            "state": dstate, "event_id": camp.id, "group_name": g.name,
            "date_start": START, "date_end": START + timedelta(days=DAYS),
            "wychowawca_ids": [(6, 0, [wychowawcy[0].id, wychowawcy[1].id])],
        })
    print("SEED dzienniki=%s" % len(groups))
    ...
    for ch in children:
        ...
        if p.email and not env["res.users"].search([("login", "=", p.email)], limit=1):
            env["res.users"].with_context(no_reset_password=True).create({
                "name": p.name, "login": p.email, "email": p.email,
                "groups_id": [(6, 0, [portal_gid])],
            })
        w = gi2w[ch["gi"]]
        wu = w_users[w.id]
        c1 = Channel.create({"name": "Rodzic %s ↔ %s" % (p.name, w.name), "channel_type": "group"})
        c1.add_members(partner_ids=[p.id, wu.partner_id.id])
        c1.message_post(body="Dzień dobry, mam pytanie o dziecko. " + LOREM_SHORT,
                        author_id=p.id, message_type="comment", subtype_xmlid="mail.mt_comment")
        c2 = Channel.create({"name": "Rodzic %s ↔ Kierownik" % p.name, "channel_type": "group"})
        c2.add_members(partner_ids=[p.id, kier_user.partner_id.id])
        c2.message_post(body="Witam Kierownika obozu. " + LOREM_SHORT,
                        author_id=p.id, message_type="comment", subtype_xmlid="mail.mt_comment")
        chat_n += 2
    print("SEED chat_channels=%s (%s rodziców ↔ wychowawca + ↔ kierownik)" % (chat_n, len(seen)))

    env.cr.commit()
    print("SEED DONE event_id=%s name=%s" % (camp.id, CAMP_NAME))
