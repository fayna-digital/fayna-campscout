...
class TestScaffold(TransactionCase):
    ...
    is seeded correctly. Real behaviour arrives in Phase 7 increments
    per master TZ §16."""

    def test_feature_flag_seeded_as_disabled(self):
        param = self.env["ir.config_parameter"].sudo().get_param("fayna_campscout.active")
        self.assertEqual(param, "False")

    def test_module_installed(self):
        mod = self.env["ir.module.module"].search([("name", "=", "fayna_campscout")], limit=1)
        self.assertTrue(mod)
